...
async def tesla_status():
    ...
    transport_status = transport_manager.get_status()
    state = transport_manager.active.last_state
    return {
        "connected": bool(token),
        "vehicle": vehicle_state_to_dict(state),
        "key_revoked": transport_manager.active.key_revoked,
...
async def set_channel(data: dict):
    ...
    try:
        transport_manager.set_channel(channel)
        elog(f"Tesla channel switched to: {channel}", INFO, "system")
...
async def tesla_command(data: dict):
    ...
    try:
        if cmd == "start":
            ...
            ok = await tesla.set_charging_amps(amps)
            ...
            ok = await tesla.set_charge_limit(percent)
            ...
            ok = await tesla.charge_port_door_open()
            ...
            ok = await tesla.charge_port_door_close()
            ...
            ok = await tesla.set_temps(driver, passenger)
            ...
            ok = await tesla.actuate_trunk("rear")
            ...
            ok = await tesla.actuate_trunk("front")
            ...
            ok = await tesla.set_sentry_mode(True)
            ...
            ok = await tesla.set_sentry_mode(False)
            ...
            ok = await tesla.window_control("vent")
            ...
            ok = await tesla.window_control("close")
            ...
            ok = await tesla.set_seat_heater(seat, level)
            ...
            ok = await tesla.set_steering_wheel_heater(on)
            ...
            ok = await tesla.set_preconditioning_max(on)
            ...
            tesla.clear_key_revoked()
            return {"ok": True, "command": cmd}
        else:
            return {"error": f"Unknown command: {cmd}"}
    except NotImplementedError:
        return {"error": f"Command '{cmd}' not supported by active transport ({transport_manager.active_channel})"}
...
async def get_full_vehicle_data():
    ...
    channel = transport_manager.active_channel
    state = transport_manager.active.last_state

    # For Fleet API, require a valid token
    if channel == "fleet_api" and not get_valid_token():
        return {"connected": False, "data": None, "vehicle_state": "unknown",
                "vehicle": None, "channel": channel}

    try:
        data = await transport_manager.active.get_full_vehicle_data()
    except NotImplementedError:
        data = None
